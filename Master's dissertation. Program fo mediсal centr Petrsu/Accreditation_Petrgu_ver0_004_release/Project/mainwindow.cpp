...
void MainWindow::on_actionAddTimer_triggered()
{
    ...
    if (servid1Point.isNull() == false)
    {
        qDebug() << "cid1Point не пуст";
        connect(cid1, SIGNAL( TimerToServerSignal(int, QVector <QString>) ), servid1, SLOT( ServerInterface(int, QVector <QString>) ));
    }
    else if (clid1Point.isNull() == false)
    {
        qDebug() << "cid1Point не пуст";
        connect(clid1, SIGNAL(StartTimer(int )), cid1, SLOT(textForNumberCickles(int )) );
        connect(clid1, SIGNAL(PauseTimer()), cid1, SLOT(PriosOdinCikle()) );
        connect(clid1, SIGNAL(UnpauseTimer()), cid1, SLOT(VozobOdinCikle()) );
        connect(clid1, SIGNAL(FinishTimer()), cid1, SLOT(PrervOdinCikle()) ); //???

        connect(clid1, SIGNAL(StartTimerMany(int )), cid1, SLOT(textForNumberCicklesMany(int )) );
        connect(clid1, SIGNAL(PauseTimerMany()), cid1, SLOT(PriosMnogoCikle()) );
        connect(clid1, SIGNAL(UnpauseTimerMany()), cid1, SLOT(VozobMnogoCikle()) );
        connect(clid1, SIGNAL(FinishTimerMany()), cid1, SLOT(PrervMnogoCikle()) );
    }
}
...
void MainWindow::on_actionAddCamera_triggered()
{
    ...
    {
        qDebug() << "cid1Point не пуст";
        connect(ptzid1, SIGNAL(setAutomateSignal(int, QVector <QString>) ), servid1, SLOT( ServerInterface(int, QVector <QString>) ));
    }
    else if (clid1Point.isNull() == false)
    {
    connect(clid1, SIGNAL(setAutomateSignal(QString )), ptzid1, SLOT(setOrunsetAutomateSlot(QString )) );
    }
}
...
void MainWindow::selectClientOrServerslot(bool ClientRole, bool ServerRole)
{
    ...
    if ( (clid1Point.isNull() == true) && (servid1Point.isNull() == true) )
    {
        if (ClientRole == true)
        {
        ...
        else if (ServerRole == true) /*Если нажата serverRole, и не нажата clientRole, то открываем окно Server*/
        {
            servid1 = new ServerForm(mdiArea);
            servid1Point = servid1;
            // Добавляем его в область
